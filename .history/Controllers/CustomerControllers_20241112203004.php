    ...
    class CustomerControllers {
        ...
        public function customer_paymentsuccess() {
            if (isset($_SESSION['is_login']) && $_SESSION['is_login'] === true && isset($_SESSION['cus_id'])) {
                ...
                // Kiểm tra và lấy thông tin thanh toán từ cổng thanh toán
                if (isset($_GET['vnp_Amount'], $_GET['vnp_BankCode'], $_GET['vnp_BankTranNo'], $_GET['vnp_CardType'], $_GET['vnp_OrderInfo'], $_GET['vnp_TransactionNo'], $_GET['vnp_TransactionStatus'], $_GET['vnp_TxnRef'], $_GET['vnp_SecureHash'], $_GET['vnp_PayDate'], $_GET['vnp_ResponseCode'], $_GET['vnp_TMNCode'])) {
                    ...
                    // Lấy Inv_ID từ session
                    if (isset($_SESSION['Inv_ID'])) {
                        $inv_ID = $_SESSION['Inv_ID'];
        
                        // Tạo mảng dữ liệu để truyền vào model
                        $paymentData = [
                            'vnp_Amount' => $vnp_Amount,
                            ...
                            'vnp_PayDate' => $vnp_PayDateFormatted,
                            'vnp_ResponseCode' => $vnp_ResponseCode,
                            'vnp_TMNCode' => $vnp_TMNCode,
                            'inv_ID' => $inv_ID // Thêm Inv_ID vào mảng dữ liệu
                        ];
        
                        // Gọi phương thức saveVNPayPayment với mảng dữ liệu
                        $customerModel->saveVNPayPayment($paymentData);
                    }
                }
