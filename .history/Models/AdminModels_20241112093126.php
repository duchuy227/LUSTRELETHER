<?php 
    ...
    $sql = $this->conn->prepare($query);
    $sql->execute();
    return $sql->fetchAll(PDO::FETCH_ASSOC); // Đảm bảo trả về tất cả dữ liệu
}
        ...
        public function getBookingbyID($booking_id) {
            $query = "SELECT * FROM Booking
                      ...
                      LEFT JOIN Invoice ON Booking.Inv_ID = Invoice.Inv_ID
                      LEFT JOIN Influencer ON Booking.Influ_ID = Influencer.Influ_ID
                      WHERE Booking.Booking_ID = :booking_id"; // Lọc theo Booking_ID
            $sql = $this->conn->prepare($query);
            $sql->bindParam(':booking_id', $booking_id, PDO::PARAM_INT); // Ràng buộc tham số
            $sql->execute();
            return $sql->fetch(PDO::FETCH_ASSOC); // Trả về một dòng dữ liệu booking
        }
