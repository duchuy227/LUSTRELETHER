<?php 
    ...
    class CustomerModels {
        ...
        public function saveVNPayPayment($vnp_Amount, $vnp_BankCode, $vnp_BankTranNo, $vnp_CardType, $vnp_OrderInfo, $vnp_PayDate, $vnp_ResponseCode, $vnp_TMNCode, $vnp_TransactionNo, $vnp_TransactionStatus, $vnp_TxnRef, $vnp_SecureHash){
            // Kiểm tra xem session đã có Inv_ID chưa
            if (!isset($_SESSION['Inv_ID'])) {
                echo "Error: No invoice ID in session.";
                return;  // Dừng lại nếu không có Inv_ID
            }
            $this->conn->beginTransaction();
            $inv_id = $_SESSION['Inv_ID'];
            $query = "INSERT INTO VNPay_Transaction (VNP_Ammount, VNP_BankCode, VNP_BankTranNo, VNP_CardType, VNP_OrderInfo, VNP_PayDate, VNP_Respond_code, VNP_TMNCode, VNP_TransactionNo, VNP_TransactionStatus, VNP_TxnRef, VNP_SecureHash, Inv_ID) VALUES(:vnp_amount, :vnp_bank_code, :vnp_bank_tranno, :vnp_card_type, :vnp_order_info, :vnp_pay_date, :vnp_response_code, :vnp_tmn_code, :vnp_transaction_no, :vnp_transaction_status, :vnp_txn_ref, :vnp_txn_ref, :inv_id)";
            $stmt = $this->conn->prepare($query);
            $stmt->execute(array(
            ...
            $vnpTransactionID = $this->conn->lastInsertId();
            if (!$vnpTransactionID) {
                echo "Error: Could not get last insert ID.";
                return;
            }

            $query_update_invoice = "UPDATE Invoice SET VNPay_ID = :vnp_transaction_id, Inv_Status = 'Paid' WHERE Inv_ID = :inv_id";
            $sql_update_invoice = $this->conn->prepare($query_update_invoice);
            $sql_update_invoice->execute(array(
                ':vnp_transaction_id' => $vnpTransactionID,
                ':inv_id' => $inv_id // Sử dụng ID hóa đơn từ session
            ));
