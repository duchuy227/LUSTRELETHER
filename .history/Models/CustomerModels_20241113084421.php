<?php 
    ...
    class CustomerModels {
        ...
        public function getTransactionDetailsByInvoiceID($inv_id, $cus_id){
            $query = "SELECT Invoice.*, Momo_Transaction.*, VNPay_Transaction.*, Booking.*,         Influencer.Influ_Fullname AS Influencer_Name, Cus.Cus_Fullname AS Customer_Name
            FROM Invoice
              INNER JOIN 
                Booking ON Invoice.Booking_ID = Booking.Booking_ID
              LEFT JOIN 
                Momo_Transaction ON Invoice.MT_ID = Momo_Transaction.MT_ID
              LEFT JOIN 
                VNPay_Transaction ON Invoice.VNPay_ID = VNPay_Transaction.VNPay_ID
              INNER JOIN 
                Influencer ON Booking.Influ_ID = Influencer.Influ_ID
              INNER JOIN 
                Cus ON Booking.Cus_ID = Cus.Cus_ID
              WHERE 
                Booking.Cus_ID = :cus_id AND Invoice.Inv_ID = :inv_id";
            ...
            if (!empty($invoice['MT_ID'])) {
                $query = "SELECT * FROM Momo_Transaction WHERE MT_ID = :mt_id";
                $sql = $this->conn->prepare($query);
                $sql->execute([':mt_id' => $invoice['MT_ID']]);
                $transaction = $sql->fetch(PDO::FETCH_ASSOC);
                $transaction['method'] = 'Momo';
            } 
            // Nếu hóa đơn có VNP_ID, lấy thông tin từ bảng VNPay_Transaction
            elseif (!empty($invoice['VNP_ID'])) {
                $query = "SELECT * FROM VNPay_Transaction WHERE VNPay_ID = :vnpay_id";
                $sql = $this->conn->prepare($query);
                $sql->execute([':vnpay_id' => $invoice['VNPay_ID']]);
                $transaction = $sql->fetch(PDO::FETCH_ASSOC);
            ...
            // Trả về thông tin của cả hóa đơn và giao dịch
            return [
                'invoice' => $invoice,
