<?php 
    ...
    class InfluencerModels {
        ...
        public function TimeLine($influ_id) {
            // Cập nhật câu truy vấn để bao gồm các trạng thái 'Approval', 'In Progress', và 'Completed'
            $query = "SELECT * FROM Booking WHERE Booking_Status IN ('Approval', 'In Progress', 'Completed') AND Influ_ID = :influ_id";
            $stmt = $this->conn->prepare($query);
            $stmt->execute([
                ':influ_id' => $influ_id
            ]);
            ...
            foreach ($bookings as $booking) {
                // Lấy ngày bắt đầu và kết thúc từ cơ sở dữ liệu
                $startDate = $booking['Booking_StartDate'];  // Đảm bảo ngày ở định dạng 'YYYY-MM-DD'
                $endDate = $booking['Booking_EndDate'];      // Đảm bảo ngày ở định dạng 'YYYY-MM-DD'
        
                // Kiểm tra nếu giá trị ngày bắt đầu và kết thúc hợp lệ
                if ($startDate && $endDate) {
                    try {
                        // Tạo đối tượng DateTime cho ngày bắt đầu và kết thúc
                        $start = new DateTime($startDate);  
                        $end = new DateTime($endDate);
                        
                        // Kiểm tra nếu ngày bắt đầu và ngày kết thúc giống nhau, chỉ hiển thị một ngày
                        if ($start == $end) {
                            $timeline[] = [
                                'Date' => $start->format('d/m/Y'), // Hiển thị ngày theo định dạng 'DD/MM/YYYY'
                                'Booking_Content' => $booking['Booking_Content'],
                                'Booking_Status' => $booking['Booking_Status']
                            ];
                        } else {
                            // Hiển thị ngày bắt đầu và kết thúc nếu chúng khác nhau
                            $timeline[] = [
                                'Date' => $start->format('d/m/Y') . ' - ' . $end->format('d/m/Y'), // Hiển thị theo định dạng 'DD/MM/YYYY'
                                'Booking_Content' => $booking['Booking_Content'],
                                'Booking_Status' => $booking['Booking_Status']
                            ];
                        }
                    } catch (Exception $e) {
                        // Log lỗi nếu có vấn đề với định dạng ngày
                        error_log('Invalid date format: ' . $e->getMessage());
                    }
                } else {
                    // Log nếu ngày bắt đầu hoặc ngày kết thúc không hợp lệ
                    error_log('Invalid date value: ' . $startDate . ' - ' . $endDate);
                }
            }
