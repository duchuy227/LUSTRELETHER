...
<style>
    ...
    .past.completed {
        background-color: #069603; /* Màu xanh lá cây */
        opacity: 0.5; /* Làm mờ */
    }
    ...
    .day.booked.past {
        ...
        opacity: 1 !important;  /* Hủy opacity của lớp past nếu có lớp booked */
    }
...
<script>
    ...
    function renderCalendar() {
        ...
        for (let i = 1; i <= lastDayDate; i++) {
            ...
            const isPast = currentDay < new Date(); // Check if it's in the past
            ...
            if (isPast) {
                ...
                if (isCompleted) {
                    dayClass = "completed";  // Chỉ thêm lớp completed nếu booking đã hoàn thành
                }
                dayClass += " past";  // Thêm lớp past cho các ngày đã qua
            }

            if (isBooked) {
                dayClass += " booked";  // Thêm lớp booked cho các ngày đã được đặt
            }
            ...
            if (i === new Date().getDate() && currentMonth === new Date().getMonth() && currentYear === new Date().getFullYear()) {
                daysHTML += `<div class="day today ${isBooked ? 'booked' : ''}">${i}</div>`;
            } else {
                daysHTML += `<div class="day ${isBooked ? 'booked' : ''} ${dayClass}">${i}</div>`;
            }
        }
