...
<script>
    ...
    // Cập nhật thông tin booking vào phần tử #Booking_Content
    function showBookingContent(date) {
        const bookingContent = document.getElementById('Booking_Content');
        const booking = completedBookings.find(booking => booking['Booking_StartDate'] === date);
        // Kiểm tra nếu có thông tin cho ngày đó
        if (bookingDetails[date]) {
            bookingContent.innerHTML = `<strong>Booking for ${date}:</strong> ${bookingDetails[date]}`;
            bookingContent.style.display = 'block'; // Hiển thị phần tử
        } else {
            bookingContent.innerHTML = 'No booking information available for this date.';
            bookingContent.style.display = 'block'; // Hiển thị phần tử
        }
    }

    // Ẩn thông tin khi di chuột ra ngoài
    ...
    function renderCalendar() {
        ...
        for (let i = 1; i <= lastDayDate; i++) {
            ...
            daysHTML += `<div class="day ${dayClass}" data-date="${currentYear}-${currentMonth + 1}-${i}">
                            ${i}
                        </div>`;
        }
        ...
        // Thêm sự kiện cho các ngày đã booking
        const bookedDays = document.querySelectorAll('.day.booked');
        bookedDays.forEach(day => {
            day.addEventListener('mouseover', function() {
                const date = day.getAttribute('data-date');
                showBookingContent(date); // Hiển thị thông tin booking
            });
            day.addEventListener('mouseout', hideBookingContent); // Ẩn thông tin khi di chuột ra ngoài
        });
    }
