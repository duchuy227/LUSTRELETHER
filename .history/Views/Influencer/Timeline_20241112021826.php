...
<script>
    ...
    function renderCalendar() {
        ...
        for (let i = 1; i <= lastDayDate; i++) {
            ...
            // Kiểm tra nếu ngày này nằm trong bất kỳ phạm vi booking nào
            bookings.forEach(booking => {
                if (isDateInRange(currentDay, booking['Booking_StartDate'], booking['Booking_EndDate'])) {
                    isBooked = true; // Nếu ngày nằm trong phạm vi booking, đánh dấu là booked
                }
            });
            ...
            if (isPast) {
                const isCompleted = completedBookings.some(booking => {
                    return isDateInRange(currentDay, booking['Booking_StartDate'], booking['Booking_EndDate']);
                });
                if (isCompleted) {
